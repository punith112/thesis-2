...
def dominant_color(fist, image):
    ...
    '''
    x,y,w,h = fist
    print((y+h)//2-5,(y+h)//2+5, (x+w)//2-5,(x+w)//2+5)
    center = image[(y+h)//2-5:(y+h)//2+5, (x+w)//2-5:(x+w)//2+5]
    print(center)
    hsvcenter = cv2.cvtColor(center, cv2.COLOR_BGR2YCR_CB)
    success = center != []
    return (success, hsvcenter)
...
def find_fist_color(image):
    ...
    if len(fists) != 0:
        ...
        dcolor = dominant_color(maxFist, image)
    return dcolor
...
while True:
    ...
    if color_detected == False:
        succ, dcolor = find_fist_color(frame)

        if succ:
            color_detected = True
            #color = np.uint8([[[int(round(dcolor[0])), int(round(dcolor[1])), int(round(dcolor[2]))]]])
            ...
            for x in range(0,10):
                for y in range(0, 10):
                    if dcolor[x][y][1] > maxcr:
                        maxcr = dcolor[x][y][1]
                    if dcolor[x][y][1] < mincr:
                        mincr = dcolor[x][y][1]
                    if dcolor[x][y][2] > maxcb:
                        maxcb = dcolor[x][y][2]
                    if dcolor[x][y][2] < mincb:
                        mincb = dcolor[x][y][2]
